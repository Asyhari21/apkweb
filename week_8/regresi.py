...
def main():
	...
	html_templ = """
	<div style="background-color:#FFD369;padding:10px;">
...
# Salary Determination CHOICE
	if choice == 'Penentuan Gaji Karyawan':
		...
		if st.button("Proses"):
			...
			regressor = load_prediction_model("score.pkl")
			experience_reshaped = np.array(experience).reshape(-1,1)

			predicted_salary = regressor.predict(experience_reshaped)

			st.info("Gaji untuk karyawan dengan pengalaman bekerja selama {} tahun: {}".format(experience,(predicted_salary[0][0].round(2))))
